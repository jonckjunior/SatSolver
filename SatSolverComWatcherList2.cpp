...
class SatSolver{
    ...
    vector< set<int> > watcher_to_clausula;
    
    public:
        SatSolver(int _numVar){
            numVar = _numVar;
            modelo.assign(numVar+1, NOVALUE);
            watcher_to_clausula.assign(2*numVar+5, set<int>());
        }
        ...
        bool propagate(int p, int nivel){
            ...
            set<int> clausula_notp = watcher_to_clausula[ indexOffSet(notp) ];
            for(int clausula : clausula_notp){
                ...
                if(newWatcher != 0 && (stateWatcher2 || modelo[abs(watcher2)] == NOVALUE)){
                    watcher_to_clausula[ indexOffSet(newWatcher) ].insert(clausula);
                    clausula_to_watcher[clausula] = pair<int,int>(newWatcher, watcher2);
                    watcher_to_clausula[ indexOffSet(watcher1) ].erase(clausula);
                }
        ...
        void addClausula(vector<int>& clausula){
            ...
            watcher_to_clausula[ indexOffSet(watcher1)].insert(idxClausula);
            watcher_to_clausula[ indexOffSet(watcher2)].insert(idxClausula);
